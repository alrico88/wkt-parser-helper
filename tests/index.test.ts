...
test('Setting the type to wkb should return Buffer', () => {
  const buffer = convertToWK(testFeature, 'wkb');
  expect(buffer instanceof Buffer).toBe(true);
});

test('Setting the type to wkt should return string', () => {
  const wkt = convertToWK(testFeature, 'wkt');
